...
def reference_rope(
    ...
    cos_basis = cos_freqs[:current_seq_len, :] if offset is None else cos_freqs[offset, :]
    sin_basis = sin_freqs[:current_seq_len, :] if offset is None else sin_freqs[offset, :]
    ...
    return output.reshape(batch_size, current_seq_len, num_heads, dims).to(dtype=x.dtype)
...
def reference_qwen2_attention(
    ...
    projection_q = reference_linear(x, wq, bias=bq).reshape(
        batch_size, seq_len, num_heads, head_dim
    )
    projection_k = reference_linear(x, wk, bias=bk).reshape(
        batch_size, seq_len, num_kv_heads, head_dim
    )
    projection_v = reference_linear(x, wv, bias=bv).reshape(
        batch_size, seq_len, num_kv_heads, head_dim
    )
    projection_q = reference_rope(
        projection_q,
        seq_len=max_seq_len,
        base=theta,
        traditional=False,
        offset=slice(0, seq_len),
    ).transpose(1, 2)
    projection_k = reference_rope(
        projection_k,
        seq_len=max_seq_len,
        base=theta,
        traditional=False,
    ...
    output = reference_grouped_attention(
    ...
    return reference_linear(output, wo)
