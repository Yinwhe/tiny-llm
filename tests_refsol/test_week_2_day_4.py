...
def assert_causal_mask_faster_than_all_zero_mask(
    ...
    # CPU microbenchmarks are noisy, especially once tensor setup and cache
    # effects dominate. We only require that the causal fast path does not
    # regress badly compared with an equivalent all-zero mask.
    assert causal_time <= zero_mask_time * 1.25, (
        "Expected causal mask fast path to stay within 25% of an all-zero mask, got "
